...
import { fetchUserOrders } from "@/app/actions"
...
const UserOrders = () => {
    ...
    useLayoutEffect(() => {
        const getOrders = async () => {
            ...
            const c = await fetchUserOrders(id, localStorage.getItem("token"))
            console.log(c)
            if (c.success) {
                setOrders(c.orders)
                setIsLoading(false)
        ...
        <section>
            <div className="mx-auto max-w-screen-xl px-4 py-8 sm:px-6 sm:py-12 lg:px-8">
                <div className="mx-auto max-w-3xl">
                    <header className="text-center flex justify-start gap-5 items-center my-3">
                        <Link href={"/admin/dashboard/users"} className="left"><MoveLeft className='text-gray-600' /></Link>
                        <h1 className="text-xl font-bold text-gray-900 sm:text-3xl">User Orders</h1>
                    </header>
